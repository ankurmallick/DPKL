...
class EmbedModel(tf.keras.Model):
    def __init__(self,latent_dim):
        ...
        # self.enc_fc1 = tf.keras.layers.Dense(200)
        # self.enc_fc2 = tf.keras.layers.Dense(200)
        self.enc_fc3 = tf.keras.layers.Dense(self.latent_dim)
        
    def embed(self, x):
        h = x
        # h = tf.nn.relu(self.enc_fc1(h))
        # h = tf.nn.relu(self.enc_fc2(h))
        return tf.nn.relu(self.enc_fc3(h))
