...
from computations import get_logits,get_accuracy,get_embedders,svgd_update
...
def train_step(X,y,embedder,weights_list,optimizer,num_lab):
    ...
    #Embedder Update
    embedder_grads = tape.gradient(negll,embedder.variables)
    optimizer.apply_gradients(zip(embedder_grads,embedder.variables))
    #Weights update
    ...
    return embedder, weights_list, optimizer

def val_step(X_val,y_val,embedder,weights_list):
    probs_list = []
    for weights in weights_list:
...
def train(X,y,X_val,y_val):
    ...
    embedder = get_embedders(num_samples,latent_dim)[0]
    ...
    for epoch in range(num_epochs):
        ...
        for (batch, (points, targets)) in enumerate(dataset):
            embedder, weights_list, optimizer = train_step(points,targets,embedder,weights_list,optimizer,num_lab)
        #Validation
        if epoch%10 == 0 or epoch == num_epochs-1:
            val_acc_check = val_step(X_val,y_val,embedder,weights_list)
            # print ([epoch, val_acc_check])
            if val_acc_check >= val_acc:
                val_acc = val_acc_check
                best_embedder = embedder.copy(X_val)
                best_weights_list = [weights.numpy() for weights in weights_list]
            else:
                continue #Don't save if performance worsens
    return best_embedder, best_weights_list
...
for trial in range(num_trials):
    ...
    while num_lab<=num_lab_max:
        ...
        embedder, weights_list = train(X,y,X_val,y_val)  
...
acc_filename = 'Results/acc_dblr.npy'
np.save(acc_filename,acc)
negll_filename = 'Results/negll_dblr.npy'
